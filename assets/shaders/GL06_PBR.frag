...
void main()
{
	...
	// use hardcoded directional light source, vec3(-1, -1, -1), with pure white light vec3(1.0)
	color += calculatePBRLightContribution(pbrInputs, normalize(vec3(-1.0, -1.0, -1.0)), vec3(1.0) );

	// multiply the color by the ambient occlusion factor. if no ambient occlusion is available, use 1.0
	color = color * (Kao.r < 0.01 ? 1.0 : Kao.r );

	// add the emissive color contribution making sure the input emissive texture is converted into the linear color space before use 
