...
{
	/// Boilerplate setup
	glfwSetErrorCallback ( []( int err, const char* description ) { fprintf ( stderr, "Error: %s\n", description ); } );
	...
	GLFWwindow* window = glfwCreateWindow ( 1024, 768, "GLAPI Wrapping Example", nullptr, nullptr );
	if ( !window )
	{
		glfwTerminate ();
		exit ( EXIT_FAILURE );
	...
	// declare our API
	GL4API api;
	// use glfwGetProcAddress() to retrieve pointers to OpenGL functions
...
#ifdef _DEBUG
	api.glGetShaderiv ( shaderVertex, GL_COMPILE_STATUS, &success );

	if ( !success )
	{
		api.glGetShaderInfoLog ( shaderVertex, 512, nullptr, infoLog );
...
#ifdef _DEBUG
	api.glGetShaderiv ( shaderFragment, GL_COMPILE_STATUS, &success );

	if ( !success )
	{
		api.glGetShaderInfoLog ( shaderFragment, 512, nullptr, infoLog );
...
#ifdef _DEBUG
	api.glGetProgramiv ( program, GL_LINK_STATUS, &success );
	if ( !success )
	{
		api.glGetProgramInfoLog ( program, 512, nullptr, infoLog );
