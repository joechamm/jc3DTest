...
int main()
{
	...
	GLShader shdFullScreenQuad(appendToRoot("assets/shaders/GL08_FullScreenQuad.vert").c_str());

	GLShader shdCombineHDR(appendToRoot("assets/shaders/GL08_HDR.frag").c_str());
	GLProgram progCombineHDR(shdFullScreenQuad, shdCombineHDR);
	...
	GLProgram progBlurX(shdFullScreenQuad, shdBlurX);
	GLProgram progBlurY(shdFullScreenQuad, shdBlurY);

	GLShader shdToLuminance(appendToRoot("assets/shaders/GL08_ToLuminance.frag").c_str());
	GLProgram progToLuminance(shdFullScreenQuad, shdToLuminance);

	GLShader shdBrightPass(appendToRoot("assets/shaders/GL08_BrightPass.frag").c_str());
	GLProgram progBrightPass(shdFullScreenQuad, shdBrightPass);

	GLShader shdAdaptation(appendToRoot("assets/shaders/GL08_Adaptation.comp").c_str());
	...
	while (!glfwWindowShouldClose(app.getWindow())) 
	{
		...
		ImGui::Checkbox("Enable HDR", &g_EnabledHDR);
		ImGui::PushItemFlag(ImGuiItemFlags_Disabled, !g_EnabledHDR);
		ImGui::PushStyleVar(ImGuiStyleVar_Alpha, ImGui::GetStyle().Alpha * g_EnabledHDR ? 1.0f : 0.2f);
		ImGui::Separator();
