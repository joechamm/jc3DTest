...
VulkanCanvas::VulkanCanvas ( VulkanRenderDevice& vkDev, VulkanImage depth ) : RendererBase ( vkDev, depth )
{
	...
	{
		if ( !createBuffer ( vkDev.device, vkDev.physicalDevice, kMaxLinesDataSize, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, storageBuffer_[i], storageBufferMemory_[i] ) )
		{
	...
	{
		printf ( "VulkanCanvas: failed to create color and depth render pass\n" );
		exit ( EXIT_FAILURE );
	}

	if ( !createUniformBuffers ( vkDev, sizeof(UniformBuffer) ) )
	{
		printf ( "VulkanCanvas: failed to create uniform buffers\n" );
		exit ( EXIT_FAILURE );
	}

	if ( !createColorAndDepthFramebuffers ( vkDev, renderPass_, depth.imageView, swapchainFramebuffers_ ) )
	{
		printf ( "VulkanCanvas: failed to create color and depth framebuffers\n" );
		exit ( EXIT_FAILURE );
	}

	if ( !createDescriptorPool ( vkDev, 1, 1, 0, &descriptorPool_ ) )
	{
		printf ( "VulkanCanvas: failed to create descriptor pool\n" );
		exit ( EXIT_FAILURE );
	}

	if ( !createDescriptorSet ( vkDev ) )
	{
		printf ( "VulkanCanvas: failed to create descriptor set\n" );
		exit ( EXIT_FAILURE );
	}

	if ( !createPipelineLayout ( vkDev.device, descriptorSetLayout_, &pipelineLayout_ ) )
	{
		printf ( "VulkanCanvas: failed to create pipeline layout\n" );
		exit ( EXIT_FAILURE );
	}
	...
	{
		printf ( "VulkanCanvas: failed to create graphics pipeline\n" );
		exit ( EXIT_FAILURE );
	}
}
...
void VulkanCanvas::updateUniformBuffer ( VulkanRenderDevice& vkDev, const mat4& modelViewProj, float time, uint32_t currentImage )
{
