...
namespace jcGLframework
{
	...
	GLTexture::GLTexture ( GLenum type, int width, int height, GLenum internalFormat )
		: type_ ( type )
	{
		int mipLevels = glm::levels<2, int> ( glm::tvec2<int> ( width, height ) );
		...
		glTextureStorage2D ( handle_, mipLevels, internalFormat, width, height );
...
//			const int numMipmaps = getNumMipMapLevels2D ( cubemap.w_, cubemap.h_ );
	...
	GLTexture::GLTexture ( GLenum type, const std::string& filename, GLenum clamp ) : GLTexture ( type, filename.c_str (), clamp )
	{}

	GLTexture::GLTexture ( GLenum type, const std::string& filename ) : GLTexture ( type, filename.c_str () )
	{}
...
//		int numMipmaps = getNumMipMapLevels2D ( w, h );
		int numMipmaps = glm::levels<2, int> ( glm::tvec2<int> ( w, h ) );
		glTextureStorage2D ( handle_, numMipmaps, GL_RGBA8, w, h );
		glTextureSubImage2D ( handle_, 0, 0, 0, w, h, GL_RGBA, GL_UNSIGNED_BYTE, img );
		glGenerateTextureMipmap ( handle_ );
		glTextureParameteri ( handle_, GL_TEXTURE_MAX_LEVEL, numMipmaps - 1 );
