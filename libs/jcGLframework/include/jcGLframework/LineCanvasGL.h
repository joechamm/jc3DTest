...
namespace jcGLframework
{
	...
	public:
		...
		void drawElements ( GLenum mode = GL_TRIANGLES ) const
		{
			...
			glBindBufferBase ( GL_SHADER_STORAGE_BUFFER, kIdxBind_Vertices, bufferVertices_->getHandle () );
	...
	class CanvasGL
	{
		...
		GLShader shdLinesVertex_ = GLShader ( appendToRoot ( "assets/shaders/jcGLframework_lines.vert" ).c_str () );
		GLShader shdLinesFragment_ = GLShader ( appendToRoot ( "assets/shaders/jcGLframework_lines.frag" ).c_str () );
