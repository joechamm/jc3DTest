	...
	: Renderer ( ctx )
{
	...
	const PipelineInfo pInfo = initRenderPass (
		PipelineInfo{
			...
			.useDepth_ = useDepth,
			.useBlending_ = false
		}, outputs, screenRenderPass, ctx.screenRenderPass_ );
	...
	for ( size_t i = 0; i < imgCount; i++ )
	{
		...
		storages_[i] = ctx.resources_.addStorageBuffer ( kMaxLinesDataSize );
	...
	initPipeline ( getShaderFilenamesWithRoot ( "assets/shaders/VK07_Lines.vert", "assets/shaders/VK07_Lines.frag" ), pInfo );
}
...
void LineCanvas::updateBuffers ( size_t currentImage )
{
	...
	uploadBufferData ( ctx_.vkDev_, storages_[currentImage].memory, 0, lines_.data (), bufferSize );
