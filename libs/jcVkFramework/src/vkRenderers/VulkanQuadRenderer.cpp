...
VulkanQuadRenderer::VulkanQuadRenderer ( VulkanRenderDevice& vkDev, const std::vector<std::string>& textureFiles ) : vkDev_ ( vkDev ), RendererBase ( vkDev, VulkanImage () )
{
	...
	for ( size_t i = 0; i < imgCount; i++ )
	{
		...
		{
			printf ( "VulkanQuadRenderer: Cannot create vertex buffer\n" );
			fflush ( stdout );
			exit ( EXIT_FAILURE );
		}
	}
	...
	{
		printf ( "VulkanQuadRenderer: Cannot create data buffers\n" );
		fflush ( stdout );
		exit ( EXIT_FAILURE );
	}
	...
	if ( !createDepthResources ( vkDev, vkDev.framebufferWidth, vkDev.framebufferHeight, depthTexture_ ) ||
		...
		!createGraphicsPipeline ( vkDev, renderPass_, pipelineLayout_, shaderFilenames, &graphicsPipeline_ ) )
	{
		printf ( "VulkanQuadRenderer: Failed to create pipeline\n" );
		fflush ( stdout );
		exit ( EXIT_FAILURE );
	}
