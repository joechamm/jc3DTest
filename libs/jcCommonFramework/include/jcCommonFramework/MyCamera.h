...
class MyCameraPositioner_FirstPerson : public AbstractCameraPositionerInterface
{
public:
	struct Movement
	{
...
public:
	MyCameraPositioner_FirstPerson () = default;
	explicit MyCameraPositioner_FirstPerson(const vec3& pos, const vec3& target, const vec3& up)
		: cameraPosition_(pos)
...
class MyCameraPositioner_OrbitTarget : public AbstractCameraPositionerInterface
{
public:
	float mouseSpeed_ = 4.0f;
	float acceleration_ = 150.0f;
...
private:
	...
	vec3 currentPosition_ = vec3 ( 0.0f, 0.0f, 10.0f );
	...
	quat currentOrientation_ = quat ( 1.0f, 0.0f, 0.0f, 0.0f );
	quat previousOrientation_ = quat ( 1.0f, 0.0f, 0.0f, 0.0f );
	mat4 viewMatrix_ = mat4 ( 1.0f );
	vec3 up_ = vec3 ( 0.0f, 1.0f, 0.0f );

public:
	MyCameraPositioner_OrbitTarget () = default;
	MyCameraPositioner_OrbitTarget ( const vec3& pos, const vec3& target, const vec3& up )
		: cameraPosition_ ( pos )
		, target_(target)
		, up_(up)
		, radius_(glm::length(target - pos))
		, cameraOrientation_ ( glm::lookAt ( pos, target, up ) )
		, previousOrientation_ ( cameraOrientation_ )
	...
	void update ( double deltaSeconds, const vec2& mousePos, bool mousePressed )
	{
		...
		// get the unit length vector pointing from the target to the camera
		vec3 unitTargetToCam = glm::normalize ( currentPosition_ - target_ );
		vec3 camForward = -unitTargetToCam;
		// extract the rotation between (0,0,1) and unitTargetToCam to orient ourselves
		quat canonicalForwardToCamDir = glm::rotation ( vec3 ( 0.0f, 0.0f, 1.0f ), unitTargetToCam );
		vec3 camRight = glm::rotate ( canonicalForwardToCamDir, vec3 ( 1.0f, 0.0f, 0.0f ) );
		up_ = glm::normalize(glm::cross ( camRight, camForward ));

		cameraPosition_ = target_ + unitTargetToCam * radius_;

		viewMatrix_ = glm::lookAt ( cameraPosition_, target_, up_ );		
	}
	...
	{
		vec3 unitLookDir = glm::normalize ( target_ - cameraPosition_ );
		cameraPosition_ = target_ + unitLookDir * r;
		viewMatrix_ = glm::lookAt ( cameraPosition_, target_, up_ );
		cameraOrientation_ = glm::quatLookAt (unitLookDir, up_ );
		previousOrientation_ = cameraOrientation_;
		currentOrientation_ = glm::quat_identity<float, glm::packed_highp> ();
		radius_ = r;
	}
	...
	inline void lookAt ( const vec3& pos, const vec3& target, const vec3& up )
	{
		const vec3 lookDir = target - pos;
		const vec3 unitLookDir = glm::normalize ( lookDir );
		up_ = glm::normalize ( up );
		radius_ = glm::length ( lookDir );
		cameraOrientation_ = glm::quatLookAt ( unitLookDir, up_ );
		previousOrientation_ = cameraOrientation_;
		currentOrientation_ = glm::quat_identity<float, glm::packed_highp> ();
		viewMatrix_ = glm::lookAt ( pos, target, up_ );
		cameraPosition_ = pos;
		target_ = target;
	}
private:
	...
	void mouseMove ( const vec2& mousePos )
	{
		...
		vec3 unitTargetToCam = glm::normalize ( cameraPosition_ - target_ );
		vec3 rotatedTargetToCam = glm::rotate ( currentOrientation_, unitTargetToCam );
		cameraPosition_ = target_ + glm::normalize ( rotatedTargetToCam ) * radius_;
		up_ = glm::rotate ( currentOrientation_, vec3 ( 0.0f, 1.0f, 0.0f ) );
